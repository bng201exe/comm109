...
if [ $# -eq 0 ]; then
    ...
    find "$BASE_DIR" -type d -name ".git" | while read GIT_DIR; do
        ...
        # inheriting the exported GITHUB_TOKEN
    ...
    # --- NEW: Verify GITHUB_TOKEN ---
    if [ -n "$GITHUB_TOKEN" ]; then
        echo "[$TARGET_DIR] GitHub Token: Found."
    else
        echo "[$TARGET_DIR] Error: GITHUB_TOKEN is not set. Exiting."
        echo "[$TARGET_DIR] Please export GITHUB_TOKEN before running the script."
    ...
    echo "[$TARGET_DIR] Setting remote URL with token..."
    REAL_ORIGIN_URL=$(git -C "$TARGET_DIR" remote get-url origin)
    
    # --- NEW: Inject token into the URL ---
    # This assumes an HTTPS URL. It replaces "https://"" with "https://YOUR_TOKEN@""
